...
test('AC-008-01/03 — informe pagina 20 y Excel conserva 120 filas autorizadas', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    assert.equal(page.json().context.lastSynchronizedAt.centro, null);
    await db.pool.query("INSERT INTO pos_cash_movements(id,shift_id,device_id,branch_id,actor_id,class,payment_method,amount,cash_delta,reason,occurred_at,data) VALUES('report-cash-movement','shift-000','centro-caja','centro',$1,'expense','cash','3000','-3000','Gasto sintético',$2,'{}')", [actor, now]);
    const cashWithMovement = await app.inject({ method: 'GET', url: '/api/reports/cash?branchId=centro&limit=100', headers }); assert.equal(cashWithMovement.statusCode, 200, cashWithMovement.body); assert.deepEqual(cashWithMovement.json().items.find((item: { id: string }) => item.id === 'report-cash-movement'), { id: 'report-cash-movement', kind: 'movement', shiftId: 'shift-000', branchId: 'centro', class: 'expense', paymentMethod: 'cash', amount: '3000.000000', cashDelta: '-3000.000000', reason: 'Gasto sintético', actorName: 'Dueño Informes', occurredAt: now.toISOString() });
    await db.pool.query("INSERT INTO inventory_items(id,name,reference,kind,base_unit) VALUES('report-item','Artículo informe','report-item','raw','unit')");
    await db.pool.query("INSERT INTO inventory_minimums(warehouse_id,item_id,minimum) VALUES('centro-venta','report-item','12')");
    const inventory = await app.inject({ method: 'GET', url: '/api/reports/inventory?branchId=centro', headers }); assert.equal(inventory.statusCode, 200, inventory.body); assert.deepEqual(inventory.json().items, [{ id: 'centro-venta:report-item', warehouseId: 'centro-venta', warehouse: 'Bodega de venta', itemId: 'report-item', name: 'Artículo informe', reference: 'report-item', quantity: '0.000000', minimum: '12.000000' }]);
    const exported = await app.inject({ method: 'GET', url: '/api/reports/cash/export?branchId=centro', headers }); assert.equal(exported.statusCode, 200, exported.body); assert.match(String(exported.headers['content-type']), /spreadsheetml/); const data = Buffer.from(exported.rawPayload); assert.equal(data.subarray(0, 2).toString(), 'PK'); const worksheet = zipEntry(data, 'xl/worksheets/sheet2.xml'); assert.equal((worksheet.match(/<row /g) ?? []).length, 122);
