...
function SupplierForm({ branchId, close, saved }: { branchId: string; close: () => void; saved: () => Promise<void> }) {
  ...
  return <Dialog className="operation-dialog" title="Nuevo proveedor" onClose={close}><SaveForm label="Guardar proveedor" onSave={async d => { await send('/suppliers', { name: text(d, 'name'), document: text(d, 'document'), contact: text(d, 'contact'), reason: text(d, 'reason') }); await saved(); }}>
