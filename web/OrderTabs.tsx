...
export function OrderTabs({ orders, selected, numbers, disabled, canCreate, amount, select, create, close }: {
  orders: OrderV2[]; selected: string; numbers: Record<string, number>; disabled: boolean; canCreate: boolean;
  amount: (order: OrderV2) => string; select: (id: string) => void; create: () => void; close: (order: OrderV2) => void;
}) {
      ...
      {orders.map((order, index) => {
        ...
        return <div className={`pos-order-tab ${active ? 'is-active' : ''}`} key={order.id}>
          <button type="button" className="order-tab-label"
            ...
            title={`${label} · ${order.lines.length} líneas · ${amount(order)}`} aria-disabled={disabled}
